...
//-------------------------------------------------------------- */
	...
	(function ($){
	   $(document).ready(function() {
		   ...
		   /* Funci—n que determina si la cadena dada representa un email valido. */
		   
		   $.fn.esCorreo = function(){
	...
	$('div.tabs').each(function() {
		...
		if ($(this).parent().prev().children('ul.sorting').length) {
			...
			$($tabs_tabs).children().children('a').click(function(){
				...
				$("#tituloBox").html("Buscar por " + $(this).html() );
				$("#criterio").val($(this).html());
				//$("#tipo").html($(this).html());
...
//-------------------------------------------------------------- */
	...
	$("input.date").datepicker( "option", "changeYear", true );
	$("input.date").datepicker( "option", "yearRange", "1960:1995" );
...
$(".box_content select, .box_content input:checkbox, .box_content input:radio, .box_content input:file, input:button, .box_content button").uniform();
...
/***************** Pide confirmacion si es que se quiere salir del sistema.*****************************/
$(".salir").click(function(){
	if( confirm("Esta seguro que quiere salir del sistema?")  ){
...
function altaProfesor(){
	...
	if( formValido( 'formRegistrarProfesor' ) ){
		...
		if( $("#email").esCorreo() ){
			...
			$.getJSON('registrarInstructor', datos, function(data) {
				
				var datos = new Array();
				datos.push( "<a><b>" + data.model.id_persona + "</b></a>" );
				datos.push( "<a><b>" + data.model.nombres + "</b></a>" );
				datos.push( "<a><b>" + data.model.apellidop + "</b></a>" );
				datos.push( "<a><b>" + data.model.apellidom + "</b></a>" );
				datos.push( "<a><b>" + data.model.email + "</b></a>" );
				...
				$("#mensaje").html('<div class="notice success"><p><b>Profesor registrado correctamente en el sistema.</b></p></div>');
			
			});
		
		}else{
		
			alert("Teclee un email valido!");
			$("#email").focus();
		}
...
/**************** Valida que todos los campos del form especificado esten llenos ***********************/
...
/**************************** Validar datos de inicio de sesi—n. **********************************/
function verificarDatos() {
	if( $("#usuario").val() != "" && $("#password").val() != ""  ){
		var formInput = $("#login").serialize();
		$.getJSON('verificarUsuario.action', formInput, function(data) {
			if( data.usuarioValido ){
				window.location = "Dashboard";
			}else{
				$("#usuarioNoValidoMensaje").html('<div class="notice warning"><p>Datos de usuario incorrectos.</p></div>');
				$("#usuario").focus();
			}
		});
	}else
		alert("Especifique usuario y contrasena.");
}
/***************************** Limpia el formulario especificado ***********************************/
